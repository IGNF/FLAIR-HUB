...
class FLAIR_MonoTemporal(torch.nn.Module):
    ...
    def __init__(self, 
        ...
        if self.return_type == 'decoder':
            if config['models']['monotemp_model']['arch'].split('_')[0] == 'smp':
                self.seg_model_head = self.seg_model.segmentation_head
                self.seg_model = self.seg_model.decoder
            elif config['models']['monotemp_model']['arch'].split('_')[0] == 'hf': 
                self.seg_model = self.seg_model.decode_head

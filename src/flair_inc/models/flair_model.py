...
class FLAIR_TimeTexture(nn.Module):
    """
    Wrapper for monotemporal and multitemporal models and fusion_module.
    """
    ...
    def process_monotemp_model(self, model, key, arch, model_keys, batch):
        if model:
            if model_keys[key] == 'full':
                if arch == 'hf':
    ...
    def decode_model(self, model, fused_feature, arch):
        if arch == 'hf':
    ...
    def forward(self, batch):
        ...
        if self.multitemp_model_sen1desc:
            _, fmaps_sen1desc = self.multitemp_model_sen1asc(
                batch['SENTINEL1-ASC_TS'], 
                batch_positions=batch['SENTINEL1-ASC_DATES']
            )
        ...
        # Process decoding for each model

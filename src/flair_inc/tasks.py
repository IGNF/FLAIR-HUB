...
from flair_inc.writer import PredictionWriter
...
def train(config, data_module, seg_module, out_dir):
    ...
    ## Define trainer
    trainer = Trainer(
        accelerator = config['hyperparams']["accelerator"],
        devices = config['hyperparams']["gpus_per_node"],
        strategy = config['hyperparams']["strategy"],
        num_nodes = config['hyperparams']["num_nodes"],

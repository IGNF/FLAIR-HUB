...
def training_stage(config, data_module, out_dir):
    ...
    print(f"\n[Training finished in {str(timedelta(seconds=inference_time_seconds))} HH:MM:SS with {config['hyperparams']['num_nodes']} nodes and {config['hyperparams']['gpus_per_node']} gpus per node]") 
